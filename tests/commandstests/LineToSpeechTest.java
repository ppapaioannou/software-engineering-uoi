...
public class LineToSpeechTest {
    ...
    public static List<Object[]> data() {
    	Object[][] data = new Object[][] { 
        	...
        	{"the text to be played is this",1},
        	
        	{"Gardening would be very fun! It wouldn't be intimidating to start growing trees.",1},
        	...
        	{"Dune is a 1965 science fiction novel by American author Frank Herbert, "
            	...
            	+ "the inaugural Nebula Award for Best Novel.\n[3] It is the first installment of the Dune saga, "
            	+ "and in 2003 was cited as the world's best-selling science fiction novel.[4][5]",3},
        	
        	{"I never got it until moving to North America."
            	+ "In my home country, temperatures seldom get so high people prefer shade, "
            	+ "and never so high people require shade.\nIt's often windy and the wind is always cold. "
            	...
            	+ "I understand the purpose of ceiling fans now. Also sandals.",4},
        	...
        	{"A story of cosmic terror about The Gardners, a family who moves "
            	...
            	+ "of the 21st century.\n\tThey are busy adapting to their new life when "
            	...
            	+ "properties of space-time with a strange, otherworldly color.",2},
        	
        	{"A secluded farm is struck by a strange meteorite which has apocalyptic "
            	+ "consequences for the family living there and possibly the world.",1},
    ...
    LineToSpeech lineToSpeech;

	@Test
	public void lineToSpeechNormalTest() {
		document.setContents(content);
		
		document.setAudioManager(fakeTSAPI);
		
		lineToSpeech = new LineToSpeech(document, "normal", lineNum);
		
		lineToSpeech.execute();
		
		assertEquals(document.getContents().get(lineNum-1).getWordsString(),fakeTSAPI.getPlay());
		
	}
	
	@Test
	public void lineToSpeechReverseTest() {
		document.setContents(content);
		
		document.setAudioManager(fakeTSAPI);
		
		lineToSpeech = new LineToSpeech(document, "reverse", lineNum);
		
		lineToSpeech.execute();
		
		Collections.reverse(document.getContents().get(lineNum-1).getWords());
	...
	@Test
	public void lineToSpeechEncodeTest() {
		document.setContents(content);
		...
		String atBash = encodingStrategy.encode(document.getContents().get(lineNum-1).getWordsString());
		
		lineToSpeech = new LineToSpeech(document, "encode", lineNum);
		lineToSpeech.execute();
		...
		String rot13 = encodingStrategy.encode(document.getContents().get(lineNum-1).getWordsString());
